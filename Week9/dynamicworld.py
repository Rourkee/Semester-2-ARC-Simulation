...
def generate_world_xml(
    ...
    *,
    ...
    # Memory sizing (dispenser now has ONE block)
    if gv is None:
        N_blocks = H * W
    else:
        N_blocks = sum(1 for r in range(H) for c in range(W) if gv[r][c] != 0)
    N_disp_blocks = 1
    N_blocks_total = N_blocks + N_disp_blocks

    N_table_lines = (H + 1) + (W + 1)
    N_geoms = 1 + 1 + 1 + 1 + N_table_lines + N_blocks_total
    nj = 1 + N_blocks_total
    ncon = max(1024, 4 * N_blocks_total + 256)
    nstack = max(30000, 80 * (N_geoms + nj))
...
'''
    ...
    # ---------- Trash bin (box shape) ----------
    bin_w, bin_d, bin_h = 0.14, 0.12, 0.12
    wall_t = 0.006
...
'''
    ...
    # ---------- Dispenser (box shape + ONE block inside) ----------
    disp_w, disp_d, disp_h = 0.14, 0.12, 0.12
    disp_z_mid = (disp_h * 0.5) + oz
    base_top = disp_z_mid - disp_h*0.5 + wall_t                      # top of dispenser base (inside)
    block_center_z = base_top + block_half + 0.001                   # block sits on base

    xml += f'''
    <!-- BLOCK DISPENSER (box like bin) -->
    <body name="block_dispenser" pos="{left_x:.3f} {left_y:.3f} {disp_z_mid:.3f}">
      <geom name="disp_base" type="box" size="{disp_w*0.5:.3f} {disp_d*0.5:.3f} {wall_t*0.5:.3f}"
            pos="0 0 {-disp_h*0.5 + wall_t*0.5:.3f}" material="disp_mat" contype="0" conaffinity="0"/>
      <geom name="disp_w1" type="box" size="{disp_w*0.5:.3f} {wall_t*0.5:.3f} {disp_h*0.5:.3f}"
            pos="0 {-disp_d*0.5 + wall_t*0.5:.3f} 0" material="disp_mat" contype="0" conaffinity="0"/>
      <geom name="disp_w2" type="box" size="{disp_w*0.5:.3f} {wall_t*0.5:.3f} {disp_h*0.5:.3f}"
            pos="0 {disp_d*0.5 - wall_t*0.5:.3f} 0" material="disp_mat" contype="0" conaffinity="0"/>
      <geom name="disp_w3" type="box" size="{wall_t*0.5:.3f} {disp_d*0.5:.3f} {disp_h*0.5:.3f}"
            pos="-{disp_w*0.5 - wall_t*0.5:.3f} 0 0" material="disp_mat" contype="0" conaffinity="0"/>
      <geom name="disp_w4" type="box" size="{wall_t*0.5:.3f} {disp_d*0.5:.3f} {disp_h*0.5:.3f}"
            pos="{disp_w*0.5 - wall_t*0.5:.3f} 0 0" material="disp_mat" contype="0" conaffinity="0"/>
...
'''
    ...
    # one normal block inside the dispenser (pickable)
    # choose a palette color 
    col_idx = palette_keys[0]
    col = palette[col_idx]

    # spawn at opening (opening_z - 0.04), same damping & contact params as the grid blocks
    ...
    xml += f'''    <body name="DISP01_b" pos="{left_x:.3f} {left_y:.3f} {disp_block_z:.3f}">
      ...
      <geom name="DISP01" type="box" size="{block_half:.3f} {block_half:.3f} {block_half:.3f}"
            rgba="{col[0]:.3f} {col[1]:.3f} {col[2]:.3f} {col[3]:.3f}"
