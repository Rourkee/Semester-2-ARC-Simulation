...
class MovementController:
    ...
    def execute_block_task(self, input_coords, output_coords, *, block_name: str = "", grid_shape=None):
        ...
        motion = [
            ("approach_input", np.linspace(xpos0, hover_input, 1500)),
            ("descend_input",  np.linspace(hover_input, input_coords, 800)),
            ("grasp",          None),
            ("ascend_input",   np.linspace(input_coords, hover_input, 800)),
            ("return_home",    np.linspace(hover_input, xpos0, 1500)),
            ("approach_output",np.linspace(xpos0, hover_output, 1500)),
            ("descend_output", np.linspace(hover_output, output_coords, 800)),
            ("release",        None),
            ("ascend_output",  np.linspace(output_coords, hover_output, 800)),
            ("final_home",     np.linspace(hover_output, xpos0, 800)),
        ]
