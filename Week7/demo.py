...
class Demo:
    ...
    def __init__(self):
        # Build a small default world so the viewer can open
    ...
    def step(self):
        ...
        for i, input_grid in enumerate(inputs):
            try:
                ...
                # 3) Plate-level rotation precheck
                def _is_rotate_rule(r):
                    return isinstance(r, str) and r.strip().lower().startswith("rotate_")

                if _is_rotate_rule(rule):
                    def until_done(model, data):
                        cur = scene_to_grid(model, data, *input_grid.shape)
                        return np.array_equal(cur, expected_output)

                    delta_hint = rotation_hint_rad(rule)
                    if delta_hint is not None:
                        print(f"[rotate-precheck] hint angle (rad): {delta_hint:.6f}")

                    # Spin until the grid matches; then RELEASE + HOME to avoid lingering in place.
                    self.rotate.spin_plate_knob(
                        angle_rad=None,
                        until_done_fn=until_done,
                        post_action="release_and_home"  # <-- changed from "release_and_hover"
                    )
                    mujoco.mj_forward(self.model, self.data)

                    # Force canonical joint-space home (belt & braces)
                    self.robot.reset_joints_to_home(steps=800)
                    mujoco.mj_forward(self.model, self.data)
                ...
                # 4) Compute block-level goal (robust to off-grid removals)
                ...
                if tx is not None:
                    ...
                    for name, obj in scene.items():
                        ...
                        if 0 <= nr < H and 0 <= nc < W:
                            ...
                            # will move off-grid -> mark for removal/stash
                            to_remove.add(name)
                else:
                    # execute_rule should only include valid target cells
                    goal = execute_rule(scene, rule, grid_shape=input_grid.shape)
                    # anything present in scene but missing in goal is implicitly "no goal"
                ...
                # Pre-stash everything that has no goal (off-grid removals / static junk)
                if to_remove:
                    refresh_scene()
                    for nm in list(to_remove):
                        if nm in scene and nm not in stashed_set:
                            ...
                            stashed_set.add(nm)
                            moved.add(nm)  # treat as "handled" so it won't be re-picked
                ...
                def try_place_stashed():
                    ...
                    while progress:
                        ...
                        for nm in list(stashed_order):
                            # If no goal (off-grid), keep it stashed and skip
                            if nm not in goal:
                                continue
                            tgt = goal[nm]
                ...
                def move_block(name, seen=None):
                    ...
                    if blocking and blocking != name:
                        # If the blocker has no goal, just stash it; else recurse
                        ...
                        # If still blocking, force-stash (deadlock breaker)
                    ...
                    # Last resort: stash current and continue later
                    stash_if_needed(name)
                    try_place_stashed()
                    return False
