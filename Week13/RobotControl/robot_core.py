...
class RobotCore:
    ...
    def reset_joints_to_home(self, steps=200):
    ...
    def zero_joint_ctrl(self):
        """
        Zero commanded torques on the 7 Panda arm joints.
        Leaves gripper actuators untouched (they are usually position motors).
        Safe to call even if some actuators are missing.
        """
        for i in range(1, 8):
            try:
                self.data.actuator(f"panda_joint{i}").ctrl = 0.0
            except Exception:
                # actuator not found or not torque-controlled; ignore
                pass
